...
void set_the_key(int sw1, BYTE *pkey, int r){
  ...
  for (i=0; i<r; i++) { 
    ...
    j = sw1 ? r-1-i :i ;   
    for (k=0; k< 48 ; k++)  KS[j][k] = pkey[PC2[k] -1];
  }
  return;
}
...
int main(){
	FILE *fpp ;
	fpp = fopen("f_key.txt", "a") ;
	BYTE keys[56] = {255,1,1,255,255,1,255,255,0,1,0,1,1,255,1,0,0,255,255,1,1,255,1,1,1,0,0,255,1,0,1,1,1,0,1,255,0,0,1,1,1,1,0,0,255,1,1,255,0,1,0,1,255,0,0,1};

	for(int i=0 ; i<16384 ; i++){
		BYTE key[56] = {255,1,1,255,255,1,255,255,0,1,0,1,1,255,1,0,0,255,255,1,1,255,1,1,1,0,0,255,1,0,1,1,1,0,1,255,0,0,1,1,1,1,0,0,255,1,1,255,0,1,0,1,255,0,0,1};
		key[0] = i%2 ;
		key[3] = (i/2)%2 ;
		key[4] = (i/4)%2 ;
		key[6] = (i/8)%2 ;
		...
		BYTE plain_text[8] = {0, 0, 0, 0, 1, 1, 1, 1} ;
		BYTE cipher_text[8] = {246, 169, 40, 200, 88, 52, 174, 97} ;
		BYTE calc_text[8] ;
	
		set_the_key(0, key, 6) ;
		des(plain_text, calc_text, 6, 'N') ;
		
		int flag = 1 ;
		for(int j=0 ; j<8 ; j++){
			if(calc_text[j]!=cipher_text[j]){
				flag = -1 ;
				break ;
			}
		}
